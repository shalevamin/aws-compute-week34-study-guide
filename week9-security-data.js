...
window.studyGuide = {
  ...
  finalExam: [
    makeQuestion(
      ...
      ["חומת אש ברמת Subnet", "חומת אש ברמת אפליקציית Web", "שירות מפתחות הצפנה", "שירות DNS"],
      0,
      "NACL פועלת ברמת תת-הרשת ומסננת Inbound ו-Outbound.",
    ),
    makeQuestion(
      "מה מתאר בצורה הטובה ביותר את תפקיד Security Group?",
      ["חומת אש Stateful ברמת משאב", "שירות ניהול מפתחות", "חיבור פרטי ייעודי", "רשימת SQL"],
      0,
      "SG מוחל על משאבים כמו EC2 ו-RDS ושומר מצב.",
    ),
    makeQuestion(
      "איזה רכיב מאפשר Deny מפורש?",
      ["NACL", "Security Group", "KMS Data Key", "NAT Gateway"],
      0,
    ...
    makeQuestion(
      ...
      ["מאפשרת את כל התעבורה הנכנסת והיוצאת", "חוסמת הכל תמיד", "עובדת רק עם HTTP", "מוצמדת רק ל-EC2"],
      0,
      "Default NACL מאפשרת Inbound ו-Outbound, אבל עדיין כוללת כלל Deny סופי.",
    ),
    makeQuestion(
      "מה נכון לגבי Custom NACL חדש?",
      ["חוסם את כל התעבורה עד לכללים מפורשים", "מאפשר תמיד הכל", "Stateful", "שייך ל-WAF"],
      0,
      "Custom NACL חדש מתחיל במצב חסום.",
    ),
    makeQuestion(
      "מה עושה DDoS?",
      ["מציף שירות כדי לפגוע בזמינות", "מצפין מפתחות", "יוצר VPC", "מגדיר IAM Role"],
      0,
    ...
    makeQuestion(
      ...
      ["Shield מתמקד ב-DDoS L3/L4; WAF מסנן בקשות Web L7", "Shield מנהל מפתחות ו-WAF מנתב NAT", "אין הבדל", "WAF עובד רק עם EBS"],
      0,
      "Shield עוסק בזמינות והתקפות רשת/תחבורה, WAF עוסק בשכבת אפליקציה.",
    ),
    makeQuestion(
      "איזה שירות מתאים לחסימת SQL Injection?",
      ["AWS WAF", "NAT Gateway", "Direct Connect", "Route Table"],
      0,
    ...
    makeQuestion(
      ...
      ["לאפשר למשאבים פרטיים לצאת החוצה בלי להיחשף", "לסנן XSS", "לאחסן מפתחות ב-HSM", "לפתוח Inbound מהאינטרנט"],
      0,
      "NAT Gateway משמש בעיקר לתעבורה יוצאת מ-Private Subnet.",
    ),
    makeQuestion(
      "לאן מפנה Route Table פרטי עבור 0.0.0.0/0 בתרחיש NAT?",
      ["NAT Gateway", "Internet Gateway ישירות", "Web ACL", "KMS"],
      0,
      "Private Route Table מפנה Default Route אל NAT Gateway.",
    ),
    makeQuestion(
      "מהו KMS?",
      ["שירות ניהול מפתחות הצפנה", "חומת אש L7", "Subnet פרטי", "שירות DDoS בלבד"],
      0,
    ...
    makeQuestion(
      ...
      ["מודול אבטחת חומרה שבו נשמרים מפתחות KMS", "חוק Firewall", "API Gateway", "Route Table"],
      0,
      "המפתחות נשמרים בתוך HSMs מאובטחים ומאושרים.",
    ),
    makeQuestion(
      "מה קורה ב-Envelope Encryption לאחר שימוש במפתח הנתונים הלא מוצפן?",
      ["הוא נמחק מהזיכרון", "הוא נשמר גלוי ליד הנתונים", "הוא הופך ל-NACL", "הוא נשלח ל-WAF"],
      0,
      "המפתח הלא מוצפן משמש להצפנה מיידית ואז נמחק מהזיכרון.",
    ),
    makeQuestion(
      "איזה שירות משמש ל-Audit של שימוש במפתחות KMS?",
      ["AWS CloudTrail", "AWS Shield Standard", "NAT Gateway", "Security Group"],
      0,
